...
function changeDirection(orientation) {
	...
	direction = orientation[(idx + 1) % orientation.length];
}
...
function start() {
	...
	window.onkeydown = evt => {
		const orientation = (evt.keyCode === rightArrow) ? clockwise :
			(evt.keyCode === leftArrow) ? countercw : undefined;
		if (orientation !== undefined) { changeDirection(orientation); }
	};
